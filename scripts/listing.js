jQuery(function( )
{
	...
	$.get( '/templates/listing/menu.html', function( result )
	{
		$( result ).menu(
		{
			position :
	...
	$.when(
			...
			// Calculator
			$.get( '/templates/listing/calculator.html', function( result )
			{
				$calculator = $( result );
	...
	// Better File Types
	$.getJSON( '/scripts/listing-filetypes.json', function( result ) {
	...
	// Desktop Icons
	$.get( '/templates/listing/icons.html', function( result )
	{
		...
		$( '> .icon', $desktop ).each( function( )
		{
			...
			{
				// Analytics
				ga('send', 'event', document.URL, $icon.data('window'));

				if( $icon.data( 'window' ) === 'pongkombat' )
				{
					window.location = 'http://www.pongkombat.com/play';
				}
				else
				{
					$( '.fake-window' ).removeClass( 'active' );
